const defaultAvatarImagesMale = [
  ...
  async function fetchPeopleYouMayKnow(id) {
    try {
      const response = await fetch(
        `http://127.0.0.1:3000/api/friends/${id}/people-you-may-know`
      );
      const result = await response.json();
      suggestedFriends = result.response;
      await renderPeopleYouMayKnow();
    } catch (error) {
      console.error(error);
    }
  }
  fetchPeopleYouMayKnow(getUrlParamters().id);
